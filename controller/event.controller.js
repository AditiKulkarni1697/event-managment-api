...
async function updateEvent(req,res,next){
    ...
    const {title, date, time, desc} = req.body;
    try{
        const updatedEvent = await EventModel.findByIdAndUpdate({_id:event_id},{title, date, time, desc});

        const participant_emails = await getEmailUsingId(updatedEvent.participant_list)

        const update_mail = await sendMail(participant_emails, `The ${updatedEvent.title} has been updated`, `Hello there, \n the ${updatedEvent.title} event has been updated.\n The updated date is ${updatedEvent.date} at ${updatedEvent.time}. See you there!`) 
...
async function participant_register(req,res,next){
    ...
    try{
        ...
        await EventModel.findByIdAndUpdate({_id:event_id},{participant_list:event.participant_list})

        const registration_mail = await sendMail([user.email], `Registration successful for ${event.title}`, `Hello ${user.name}, \n Thank you for registering with the ${event.title} event.\n See you on ${event.date} at ${event.time}`)

        res.status(200).send({message:"Participant successfully registered for event"})
    }catch(err){
        res.status(500).send({message:"Internal Server Error"})
    }
}
...
async function deleteEvent(req,res,next){
    ...
    try{
        ...
        const participant_emails = await getEmailUsingId(deletedEvent.participant_list)

        const delete_mail = await sendMail(participant_emails, `The ${deletedEvent.title} event has been deleted`, `Hello there, \n We regret to inform that ${deletedEvent.title} event has been deleted.\n Kindly take the note`) 
...
module.exports = {createEvent, updateEvent, participant_register, getEvent, deleteEvent}